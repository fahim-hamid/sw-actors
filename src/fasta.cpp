...
std::string parse_identifier(std::string_view header) {
  ...
  const std::string_view::const_iterator end =
      std::find_if(header.begin(), header.end(), is_space);
  return std::string(header.begin(), end);
}

[[nodiscard]]
FastaError make_error(FastaErrorCode code, std::size_t line,
                      std::string message) {
  return FastaError{
      .code = code,
      .line = line,
...
FastaResult read_fasta(std::istream &input, std::size_t max_sequences) {
  ...
  while (std::getline(input, line)) {
    ...
    if (content.front() == '>') {
      if (has_header) {
        if (current.data.empty()) {
          return std::unexpected(make_error(
              FastaErrorCode::empty_sequence, header_line,
      ...
      if (current.id.empty()) {
        return std::unexpected(make_error(FastaErrorCode::empty_identifier,
                                          line_number,
    ...
    if (!has_header) {
      return std::unexpected(
          make_error(FastaErrorCode::sequence_before_header, line_number,
  ...
  if (input.bad()) {
    return std::unexpected(make_error(FastaErrorCode::read_failed, line_number,
  ...
  if (has_header) {
    if (current.data.empty()) {
      return std::unexpected(
          make_error(FastaErrorCode::empty_sequence, header_line,
                     "FASTA record '" + current.id + "' has no sequence data"));
    }
...
FastaResult read_fasta(const std::filesystem::path &path,
                       std::size_t max_sequences) {
  std::ifstream input(path);
  if (!input) {
    return std::unexpected(
        make_error(FastaErrorCode::open_failed, 0,
