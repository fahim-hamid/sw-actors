...
AlignmentResult traceback(std::string_view query, std::string_view subject,
                          std::span<const std::uint8_t> trace,
                          std::span<CigarOp> cigar, std::size_t columns,
                          std::size_t endpoint_row, std::size_t endpoint_column,
...
void AlignmentScratch::prepare(std::size_t maximum_query_length,
...
AlignmentResult smith_waterman(std::string_view query, std::string_view subject,
  ...
  auto previous = std::span{scratch.previous_row}.first(columns);
  auto current = std::span{scratch.current_row}.first(columns);
  auto trace = std::span{scratch.trace}.first(trace_size);
  auto cigar = std::span{scratch.cigar}.first(maximum_cigar_size);
  ...
  for (std::size_t row = 1; row < rows; ++row) {
    ...
    for (std::size_t column = 1; column < columns; ++column) {
      const std::int32_t substitution = query[row - 1] == subject[column - 1]
                                            ? scoring.match
                                            : scoring.mismatch;
      ...
      trace[trace_row + column] = static_cast<std::uint8_t>(direction);
  ...
  return traceback(query, subject, trace, cigar, columns, endpoint_row,
                   endpoint_column, maximum_score);
}
