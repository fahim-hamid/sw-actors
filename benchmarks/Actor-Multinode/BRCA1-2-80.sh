...
#SBATCH --output=MS-BRCA1-2-80cpu%j.out
...
for node in $(scontrol show hostnames); do
    ...
    if [ $counter -eq 0 ]; then
        IP=$(srun --nodes=1 --ntasks=1 hostname -i)
        srun --nodes=1 --ntasks=1 ./swActor -D -s -p 4428 -Q ../dataset/BRCA1.fasta -O BRCA1-2-40.SAM -A 40 -m 2 -M -1 -g -2 -R 1 -C 1 --caf.scheduler.max-threads=40&
        sleep 2
        counter=$((counter + 1))
    else
