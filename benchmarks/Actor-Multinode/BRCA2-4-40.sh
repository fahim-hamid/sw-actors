...
#SBATCH --output=MS-BRCA2-4-40cpu%j.out
...
for node in $(scontrol show hostnames); do
    ...
    if [ $counter -eq 0 ]; then
        IP=$(srun --nodes=1 --ntasks=1 hostname -i)
        srun --nodes=1 --ntasks=1 ./swActor -D -s -p 4444 -Q ../dataset/BRCA2.fasta -O BRCA2-2-40.SAM -A 10 -m 2 -M -1 -g -2 -R 1 -C 1 --caf.scheduler.max-threads=10&
        sleep 2
        counter=$((counter + 1))
    else
